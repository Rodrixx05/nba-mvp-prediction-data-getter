...
def get_processed_prediction(pred_val_y, games_played_series, num_contenders = 17, total_share = 2.6, max_votes = 1010):

    df_results = pd.concat([pred_val_y, games_played_series], axis = 1)

    for column in pred_val_y.columns:

        df_results.loc[df_results['%G'] < 0.793, column] = 0

        df_results.sort_values(column, ascending = False, inplace = True)

        df_results[f'{column}_Adj'] = 0.0
